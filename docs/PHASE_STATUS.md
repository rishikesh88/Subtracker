...
| **Clearing data did not clear `screened_messages`** | Fixed 2026-09-09. `/api/clear-data` wiped emails, subscriptions and suggestions but left the screened-id table intact, so the sync immediately after a clear skipped 2,597 of 2,622 messages and produced **zero** suggestions against an unchanged mailbox. Phase 4's bookkeeping has to be cleared with the data it describes. This silently invalidated the 16:19 run |
